...
moveLeftN' =
  moveLeftCount 0
  where
    moveLeftCount count n z
      | n < 0 = moveRightN' (abs n) z
      | n == 0 = Right z
      | otherwise = 
        case moveLeft z of
        IsZ next -> moveLeftCount (count + 1) (n-1) next
        IsNotZ -> Left count
...
moveRightN' =
  moveRightCount 0
  where
    moveRightCount count n z
      | n < 0 = moveLeftN' (abs n) z
      | n == 0 = Right z
      | otherwise = 
        case moveRight z of
        IsZ next -> moveRightCount (count + 1) (n-1) next
        IsNotZ -> Left count --}
...
nth n (ListZipper ls x rs)
   | n == leftLength = IsZ $ ListZipper ls x rs
   | n < leftLength = moveLeftN (leftLength - n) $ ListZipper ls x rs
   | otherwise = moveRightN (n - leftLength) $ ListZipper ls x rs
   where 
     leftLength = length ls
...
end (ListZipper l a (r :. rs)) = end $ ListZipper (a :. l) r rs
...
start (ListZipper (l :. ls) a r) = start $ ListZipper ls l (a :. r)
...
instance Apply ListZipper where
  (ListZipper fl fx fr) <*> (ListZipper al ax ar) =
    ListZipper (zipApply fl al) (fx ax) (zipApply fr ar)
    where 
      applyF f a = f a
      zipApply fs as = zipWith applyF fs as
...
instance Extend ListZipper where
  f <<= z =
    ListZipper (unfoldr leftZippers z) (f z) (unfoldr rightZippers z)
    where 
      leftZippers (ListZipper Nil _ _) = Empty
      leftZippers (ListZipper (h:.t) x r) = 
        let next = (ListZipper t h (x :. r)) 
        in Full (f next, next)
      rightZippers (ListZipper _ _ Nil) = Empty
      rightZippers (ListZipper l x (h:.t)) =
        let next = ListZipper (x:.l) h t
        in Full (f next, next)
