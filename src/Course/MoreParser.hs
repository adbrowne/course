...
tok pa =
  let 
    suckSpaces a =
      spaces >>> valueParser a
  in
    pa `flbindParser` suckSpaces 
...
between ::
  ...
  -> Parser a
  -> Parser a
between pstart pend pbody =
  pstart >>> (pbody `flbindParser` (\body ->
    (pend >>> (valueParser body))))
...
betweenCharTok ::
  ...
  -> Parser a
  -> Parser a
betweenCharTok start end pbody =
  between (charTok start) (charTok end) pbody
...
hex =
  let 
    hexDigitsParser = sequenceParser (replicate 4 (satisfy isHexDigit)) 
  in
    hexDigitsParser `flbindParser` (\cs ->
      case readHex cs of
      Full h -> valueParser $ (chr h)
      Empty -> failed
    ) 
...
sepby1 pa ps =
  let 
    asp = (list (ps >>> pa))
    appValue a as = valueParser $ a :. as
  in
    (\a -> asp >>= (appValue a)) =<< pa
...
betweenSepbyComma ::
  ...
  -> Parser a
  -> Parser (List a)
betweenSepbyComma start finish pa =
  betweenCharTok start finish (sepby pa (charTok ','))
